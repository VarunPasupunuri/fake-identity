...
export function summarise({ status, hits, serious, ocrConfidence, ran, profile }) {
  if (status === AUTHENTICITY.TAMPERED) {
    ...
    // The original value is recorded in the evidence but deliberately not named here.
    // Several values can satisfy the same check digit, and the digits used to narrow
    // them can themselves be misread, so naming one risks stating a wrong fact with
    // the authority of arithmetic. What is certain is that the value shown is not the
    // one the check digit was computed for.
    if (rebuilt) return `${FIELD_PHRASE[rebuilt.field] || 'A field'} was altered: it does not match the check digit printed beside it in the machine readable zone.`;
...
export function determineAuthenticity({ documentType = 'generic_document', ocr = null, tampering = null, face = null, barcode = null } = {}) {
  ...
  // --- 2. MRZ integrity ----------------------------------------------
  const mrzIndicators = [];
  if (mrzParsed) mrzIndicators.push(...checksumIndicators(mrzParsed, ocrConfidence, { fields: visual, dates: ocr?.printedDates || [] }));
  else if (profile.mrz) {
